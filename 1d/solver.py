...
def get_eigenpairs(mesh,Eps,nconv,Bsc,V,x0,x1,nelts,npts,plotefuns,plotefuns_2,eigenvalfile,eigenfunplotfile,eigenfunh5file,eigenfunmontagefile,eigenfunmontagefile_2,center_list=[],flag=0,eigenfunmon_all="",ncol_plot=5):
    # get eigenpairs
    eigenvalues = []
    eigenvalues_v2 = []
    pratio=[]
    ...
    if flag == 0:
       combine_images(columns=ncol_plot, space=20, images=eigenf_imgs,file=eigenfunmontagefile)
       PETSc.Sys.Print("> eigenfunction montage written to {}".format(eigenfunmontagefile)) 
       combine_images(columns=ncol_plot, space=20, images=eigenf_imgs_2,file=eigenfunmontagefile_2)
       PETSc.Sys.Print("> another eigenfunction montage written to {}".format(eigenfunmontagefile_2)) 
    else:
       nplot_total=ncol_plot*5
       dd,dd2=divmod(nconv,nplot_total)
       for i in range(dd):
           ...
           combine_images(columns=ncol_plot, space=20, images=eigenf_imgs,file=eigenfunmon_all.format(i0,iend))
